...
public class SmotheringPresence : TheScaledCard
{
    ...
    protected override IEnumerable<DynamicVar> CanonicalVars =>
        [new DynamicVar("PowerAmount", 1), new DynamicVar("ExtraConsumption", 2)];

    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        (
            await PowerCmd.Apply<SmotheringPresencePower>(
                choiceContext,
                base.Owner.Creature,
                base.DynamicVars["PowerAmount"].IntValue,
                base.Owner.Creature,
                this
            )
        )?.AddDrownConsumption(base.DynamicVars["ExtraConsumption"].IntValue);
    }
